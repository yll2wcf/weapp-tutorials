...
<view class="quick-search" wx:if="{{showSearchPanel == 1}}">
    ...
    <view class="quick-search-bd">
      <text class="quick-search-item-red" data-url="{{special.url}}" data-key="{{special.key}}" bindtap="hotKeysTap">{{special.key}}</text>
...
绑定的数据有两个，special和hotkey，同样我们需要将他们加入data里。
